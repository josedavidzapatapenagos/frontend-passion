...
export const AUTH_SESSION_KEY = AUTH_STORAGE_KEYS.session;
...
export const inferAccountTypeFromSession = async (): Promise<string | null> => {
  return normalizeAccountType(readStoredAuthState().accountType);
};
...
export const validateStoredSession = async (): Promise<boolean> => {
  ...
  try {
    if (normalizedAccountType === "ADMIN" || normalizedAccountType === "SUPER_ADMIN") {
      await apiClient.get("/v1/admin/identity-verifications/pending", {
        params: { pageNumber: 0, pageSize: 1 },
        headers: { accept: "*/*" },
      });
    } else if (normalizedAccountType === "MODEL") {
