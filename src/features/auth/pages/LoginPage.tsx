...
import { AUTH_SESSION_KEY, login, normalizeAccountType } from "@/features/auth/services/authService";
...
const extractAuthPayload = (
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    const hasAuthFields =
      ...
      Boolean(candidate.account_type) ||
...
const getAccountTypeFromAuthPayload = (
  ...
  return normalizeAccountType(sourceValue);
};
  ...
  const handleLogin = async (
    ...
    try {
      ...
      const accountName =
        ...
        authPayload.account_name ||
        authPayload.accountFullName ||
        authPayload.account_full_name ||
        authPayload.fullName ||
        authPayload.full_name ||
        email;
