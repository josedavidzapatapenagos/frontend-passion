...
import { getCatalogPosts, type Post } from '@/features/feed/services/catalogService';
import { getUserFacingErrorMessage } from '@/utils/errors/errorMapper';
import { useNotification } from '@/hooks/useNotification';
import { PostCard } from './PostCard';
import { PostDetailView } from './PostDetail';
import { PremiumStatesCarousel } from './PremiumStatesCarousel';
...
export const Feed = ({ catalogId, onPostClick, viewingPostId }: FeedProps) => {
  ...
  const handleSelectPremiumState = (postId: string) => {
    onPostClick(postId);

    requestAnimationFrame(() => {
      document
        .getElementById(`feed-post-${postId}`)
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 mt-12 space-y-10">
      <PremiumStatesCarousel
        catalogId={catalogId}
        onSelectPost={handleSelectPremiumState}
      />

      {posts.map((post) => (
        <div key={post.id} id={`feed-post-${post.id}`} className="w-full flex flex-col items-start scroll-mt-28">
          <PostCard 
            post={post} 
