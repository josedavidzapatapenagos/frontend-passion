...
import { AUTH_SESSION_KEY, normalizeAccountType } from "../services/authService";
...
const isNavbarRole = (value: string | null): value is NavbarRole => {
  return value === "USER" || value === "MODEL" || value === "ADMIN" || value === "SUPER_ADMIN";
};

const readAuthSnapshot = (): AuthSnapshot => {
  const token = localStorage.getItem("token");
  const hasSessionFlag = localStorage.getItem(AUTH_SESSION_KEY) === "true";
  const normalizedAccountType = normalizeAccountType(localStorage.getItem("accountType"));
  const isLogged = (!!token || hasSessionFlag) && Boolean(normalizedAccountType);
  ...
  const accountName =
    ...
    localStorage.getItem("accountFullName");
  const accountType = normalizedAccountType;
  return {
    accountName,
    accountType,
    isLogged,
...
export const Navbar = () => {
  ...
  const { accountApproved, hasCompletedOnboarding } = useModelAccess();

  const [modelVerificationStatus, setModelVerificationStatus] = useState<string | null>(
    localStorage.getItem("modelVerificationStatus")
  );
    ...
    role !== "MODEL"
      ...
      : normalizedMenuItems
          .map((item) => {
            ...
            if (item.path === "/ads" || item.path === "/vip") {
              if (!accountApproved) {
                return null;
              }
