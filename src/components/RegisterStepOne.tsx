...
export const RegisterStepOne = ({ onSelectionComplete }: RegisterStepOneProps) => {
  ...
  useEffect(() => {
    document.body.style.backgroundColor = "#013440";
  }, []);
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-[#013440]">
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-[#013440] text-red-400 font-bold italic px-6 text-center">
      {error}
    </div>
  );

  return (
    <div className="min-h-screen w-full p-8 flex flex-col items-center justify-center font-sans bg-[#013440]">
      ...
      <header className="mb-12 text-center animate-in fade-in slide-in-from-top-4 duration-700">
        ...
        <p className="text-white/40 mt-3 uppercase tracking-[0.4em] text-[10px] font-bold">
          Pasión virtual
        </p>
      </header>
        ...
        {typedCountries.map((country) => (
          <button
            ...
            className={`group relative flex flex-col items-center p-10 rounded-[2.5rem] border-2 transition-all duration-500 ${
              regionId === country.id 
                ? 'bg-[#012a33] border-[#FD0083] shadow-[0_0_40px_rgba(253,0,131,0.2)] scale-[1.02]' 
                : 'bg-[#012a33]/40 border-white/5 hover:border-white/20 hover:bg-[#012a33]'
            }`}
          >
            <div className={`relative w-24 h-24 mb-6 rounded-full overflow-hidden border-2 transition-all duration-500 shadow-2xl ${
              regionId === country.id ? 'border-[#FD0083] rotate-3' : 'border-white/10 group-hover:scale-105'
            }`}>
              <img 
            ...
            <span className={`text-xl font-black tracking-tight transition-colors duration-300 ${
              regionId === country.id ? 'text-white' : 'text-white/40 group-hover:text-white'
            }`}>
              {country.name}
            </span>
        ...
        <button 
          ...
          className={`w-full py-5 rounded-2xl font-black uppercase tracking-[0.3em] transition-all text-xs border-2 shadow-2xl ${
            regionId 
              ...
              : 'bg-transparent border-white/10 text-white/20 cursor-not-allowed opacity-40'
          }`}
        >
